      ...
      return (
        <div
          className={`flex items-center justify-center p-5 ${
            fullPage ? 'min-h-screen bg-linear-to-br from-primary/30 to-secondary/30' : 'w-full py-10'
          }`}
        >
          <Card className="max-w-xl w-full">
            <Card.Header>
              <Card.Title>⚠️ Something Went Wrong</Card.Title>
            </Card.Header>
            <Card.Body>
              <div className="p-6">
                ...
                {isDevelopment && error && (
                  <details className="mb-6 p-3 border border-border rounded-lg overflow-hidden">
                    <summary className="font-semibold cursor-pointer select-none text-sm">
                      Technical Details (Dev Mode)
                    </summary>
                    <pre className="mt-3 p-4 text-muted-foreground rounded font-mono text-xs overflow-auto max-h-40 shadow-inner">
                      {error.toString()}
                      {'\n\n'}
