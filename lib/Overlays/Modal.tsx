        ...
        <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
          <div className="relative transform overflow-hidden rounded-lg bg-gray-800 text-left shadow-xl outline -outline-offset-1 outline-white/10 transition-all data-closed:translate-y-4 data-closed:opacity-0 data-enter:duration-300 data-enter:ease-out data-leave:duration-200 data-leave:ease-in sm:my-8 sm:w-full sm:max-w-lg data-closed:sm:translate-y-0 data-closed:sm:scale-95">
            ...
            <button
              ...
              aria-label="Close dialog"
              className="absolute top-3 right-3 inline-flex h-8 w-8 items-center justify-center rounded-md text-gray-300 hover:text-white bg-transparent hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-400"
            >
              <svg
            ...
            <div className="px-4 pt-5 pb-4 sm:p-6 sm:pb-4 text-white">{nonFooter}</div>
            {footer}
          </div>
        </div>
      </div>
...
const Footer = ({ children }: { children: ReactNode }) => (
  <div className="bg-gray-700/25 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">{children}</div>
);
